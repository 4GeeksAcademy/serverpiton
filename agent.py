...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Usala tambien cuando necesites descubrir el ID "
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "unit": {
                        "type": "string",
                        "description": "Unidad de medida, por ejemplo unidades, bolsas, kg o litros",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "delta": {
                        "type": "number",
                        "description": "Cantidad que se suma o resta del stock",
                    },
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "umbral. Si el usuario no indica un limite, usar 10."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "threshold": {
                        "type": "number",
                        "description": "Cantidad minima considerada segura",
                    }
                },
                "additionalProperties": False,
            },
...
def api_request(method, path, data=None):
    ...
    try:
        ...
        return {
            "error": True,
            "detail": "No se pudo conectar con la API. Comproba que FastAPI este ejecutandose.",
        }
...
def execute_tool(tool_name, arguments):
    ...
    if tool_name == "get_low_stock_alerts":
        ...
        query = urlencode({"threshold": threshold})
        return api_request("GET", f"/inventory/alerts?{query}")

    return {"error": True, "detail": f"Tool desconocida: {tool_name}"}


def call_llm(memory):
    system_prompt = """
Sos un asistente de inventario para una tienda
de suministros de cafeteria.
...
- Cuando necesites informacion real del inventario,
  usa una tool.
- Para crear productos usa create_product.
- Para consultar productos usa list_inventory.
- Para consultar faltantes usa get_low_stock_alerts.
- Para modificar stock usa update_stock.
- update_stock necesita el ID del producto.
- Si el usuario menciona un producto por nombre y
  no conoces su ID, primero usa list_inventory.
- Un ingreso de mercaderia usa delta positivo.
- Una venta o salida de mercaderia usa delta negativo.
- Despues de recibir el resultado de una tool,
  analizalo y decidi si necesitas otra tool.
- Cuando ya tengas suficiente informacion,
...
def run_agent():
    ...
    while True:
        ...
        while True:
            ...
            memory.append(assistant_message.model_dump(exclude_none=True))
            ...
            final_answer = assistant_message.content or "No pude generar una respuesta."

            print(f"\nAgente: {final_answer}\n")

            log_event(actor="agent", message=final_answer)
            break
