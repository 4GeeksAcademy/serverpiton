    ...
    for product in products:
        if product["id"] == product_id:
            ...
            if new_quantity < 0:
                raise HTTPException(
                    status_code=400,
                    detail="La operacion dejaria el stock en negativo",
                )
...
def get_inventory_alerts(threshold: float = Query(default=10, ge=0)):
    products = read_products()

    alerts = [product for product in products if product["quantity"] < threshold]
